...
export const BaseInput = {
    ...
    <div v-bind:class="[inline ? 'inline-field' : 'field', grow ? 'grow' : '']">
        ...
        <input type="[number ? 'number' : type]" min="min" :value="modelValue" @input="emits && $emit('update:modelValue', $event.target.value)" v-bind="$attrs" v-bind:class="[inputClass, number ? 'input-number' : '', -emptyClass]">
    </div>
    `,
};
...
export const ElementToggleBox = {
    ...
    computed: {
        message() {
            ...
            switch (this.fields.type) {
                case "Context":
                    return `
    <p>Contexts are modules that contain functions that are specific to a particular domain or resource.</p>
    <p>They are typically used to group together functions that work with the same data.</p>
    <p>For example, if you have a User resource, you might have a User context that contains functions for creating, updating, and deleting users.</p>
    <p>When you generate a context, you need to specify the name of the context and the name of the resource that the context is related to.</p>
    <p>For example, to generate a User context, you would run the following command:</p>
    <pre>mix phx.gen.context User User name:string email:string</pre>
    <p>The first argument is the name of the context (User) and the second argument is the name of the resource (User).</p>
    <p>After the resource name, you can specify any number of fields for the resource.</p>
    <p>Each field should be followed by a colon and the data type of the field.</p>
    <p>For example, to generate a User context with a name field and an email field, you would run the following command:</p>
    <pre>mix phx.gen.context User User name:string email:string</pre>
    `;
                case "Notifier":
                    return /*html*/ `
    <section id="moduledoc">
        ...
        <pre><code class="makeup shell" translate="no"><span class="gp unselectable">$ </span><span class="">mix phx.gen.notifier Accounts User welcome_user reset_password confirmation_instructions
        ...
        <pre><code class="makeup shell" translate="no"><span class="gp unselectable">$ </span><span class="">mix phx.gen.notifier Accounts User welcome_user --context-app marketing
              </span></code><button class="copy-button"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="currentColor" data-darkreader-inline-fill="" style="--darkreader-inline-fill: currentColor;"><path d="M0 0h24v24H0z" fill="none"></path><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"></path></svg><span class="sr-only">copy</span><span aria-live="polite"></span></button></pre>
    ...
    `;
                case "migration":
                    return `
    <p>Migrations are scripts that define changes to the database schema.</`;
            }
        },
    },
    template: /*HTML*/ ` <div class="help-box" v-html="message">
        <slot></slot>
        </div>
        `,
};
