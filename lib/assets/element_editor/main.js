...
import { ContextAppForm } from "./components/ContextAppForm.js";
import { ModuleForm } from "./components/ModuleForm.js";
import { BlueprintForm } from "./components/BlueprintForm.js";
import { WebForm } from "./components/WebForm.js";
import { ElementToggleBox } from "./components/baseComponents.js";
...
export function init(ctx, info) {
  ...
  const app = Vue.createApp({
    components: {
      ...
      ContextAppForm: ContextAppForm,
      ModuleForm: ModuleForm,
      WebForm: WebForm,
      BlueprintForm: BlueprintForm,
      ElementToggleBox: ElementToggleBox,
    },
      ...
      <form @change="handleFieldChangeSelector">
        <div class="container">
          <div class="row header">
            ...
            />
            <div class="grow"></div>
            <button id="help-toggle" @click="toggleHelpBox" class="icon-button" type="button">
              <i class="ri ri-questionnaire-line" aria-hidden="true"></i>
            </button>
          </div>
          <ElementToggleBox class="info-box" v-show="!fields.isHelpBoxHidden" v-bind:fields="fields"/>
          ...
          <AppForm v-bind:fields="fields" v-if="isApp" v-bind:availableDatabaseAdapters="availableDatabaseAdapters" @change="handleFieldChangeSelector2"/>
          ...
          <NotifierForm v-bind:fields="fields" v-if="isNotifier" @change="handleListChange"/>
          ...
          <SchemaForm v-bind:fields="fields" v-if="isSchema" @change="handleListChange"/>
          <EmbeddedForm v-bind:fields="fields" v-if="isEmbedded" @change="handleListChange"/>
          <ContextForm v-bind:fields="fields" v-if="isContext"/>
          <ContextAppForm v-bind:fields="fields" v-if="isContextApp"/>
          <ModuleForm v-bind:fields="fields" v-if="isModule"/>
          <WebForm v-bind:fields="fields" v-if="isWeb"/>
          <ReleaseForm v-bind:fields="fields" v-if="isRelease"/>
    ...
    data() {
      return {
        ...
        availableElements: [
          ...
          { label: "ContextApp", value: "ContextApp"},
    ...
    computed: {
      ...
      isRelease() {
        return this.fields.type === "Release";
      },
      ...
      isContextApp() {
        return this.fields.type === "ContextApp";
      },
    ...
    methods: {
      ...
      toggleHelpBox(_) {
        ...
        const field = "isHelpBoxHidden";
        let value = !this.fields.isHelpBoxHidden;
      ...
      handleFieldChangeSelector(event) {
        ...
        if (field) {
          let value = null;
          if (["fields", "message_names"].includes(field)) {
            console.log("field: ", field, " value: ", this.fields[field]);
      ...
      handleListChange(fieldsProxy, field, newListProxy) {
        console.log("IT DOES CATCH IT", field, fieldsProxy, newListProxy);
        let value = Vue.toRaw(fieldsProxy);
        let newList = Vue.toRaw(newListProxy);
        console.log("newList: ", newList)
        switch (field) {
          case "Notifier":
            ctx.pushEvent("update_field", {field: "message_names", value: newList});
            break;
          case "Schema":
            ctx.pushEvent("update_field", {field: "fields", value: newList});
            break;
          case "Embedded":
            ctx.pushEvent("update_field", {field: "fields", value: newList});
            break;
